...
For optional placeholders, append `??` with a fallback value that is shown and printed while the placeholder is empty — but counts as zero in calculations until you actually fill in the field. With `??=` the value is considered in calculations right away instead. Define the fallback at the first occurrence of a placeholder — it then applies to all of them.
